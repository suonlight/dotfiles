;;; chatgpt-api.el --- Simple ChatGPT frontend for Emacs  -*- lexical-binding: t -*-
...
(defgroup chatgpt-api nil
  "ChatGPT frontend."
  :group 'convenience
  :prefix "chatgpt-api-")

(defcustom chatgpt-api-max-tokens 300
  "Upper limit on the number of tokens the API will return."
  :type 'integer)

(defvar chatgpt-api-buffer "*ChatGPT*"
  "Title of the buffer used to store the results of an OpenAI API query.")

(define-error 'chatgpt-api-error "An error related to the ChatGPT emacs package")

(define-error 'chatgpt-api-parsing-error
  "An error caused by a failure to parse an OpenAI API Response")

(defmacro chatgpt-api-show-results-buffer-if-active ()
  "Show the results in other window if necessary."
  `(if (and (not ;; visible
             (get-buffer-window chatgpt-api-buffer))
	    (called-interactively-p 'interactive))
       (lambda (&optional buf) (ignore buf)
         (with-current-buffer buf
           (view-mode t))
         (switch-to-buffer-other-window chatgpt-api-buffer))
     #'identity))

;;;###autoload
(defun chatgpt-api-prompt (prompt callback)
...
Returns buffer containing the text from this query"
  (interactive (list (read-string "Prompt ChatGPT with: ")
                     (lambda (buf) (with-current-buffer buf
                                (view-mode t))
                       (switch-to-buffer-other-window chatgpt-api-buffer))))
  (chatgpt-api--query-open-api prompt
                           (lambda (results)
                             (with-current-buffer (get-buffer-create chatgpt-api-buffer)
                               ;; Erase contents of buffer after receiving response
                               (read-only-mode -1)
                               (erase-buffer)
                               (insert results)
                               ;; Return the chatgpt-api output buffer for non interactive usage
                               (funcall callback (current-buffer))))))

;;;###autoload
(defun chatgpt-api-fix-region (BEG END)
  "Takes a region BEG to END asks ChatGPT to explain whats wrong with it.
It then displays the answer in the `chatgpt-api-buffer'."
  (interactive "r")
  (let ((current-code (buffer-substring BEG END)))
    (chatgpt-api-prompt (chatgpt-api--append-to-prompt
                     current-code
                     "Why doesn't this code work?")
                    (chatgpt-api-show-results-buffer-if-active))))

;;;###autoload
(defun chatgpt-api-explain-region (BEG END)
  "Takes a region BEG to END asks ChatGPT what it does.
The answer in the displays in `chatgpt-api-buffer'."
  (interactive "r")
  (let ((current-code (buffer-substring BEG END)))
    (chatgpt-api-prompt (chatgpt-api--append-to-prompt
                     current-code
...
;;;###autoload
(defun chatgpt-api-gen-tests-for-region (BEG END)
...
;; (defun chatgpt-api-optimize-region (BEG END)
...
;;;###autoload
(defun chatgpt-api-refactor-region (BEG END)
...
;;;###autoload
(defun chatgpt-api-prompt-region (BEG END)
  "Prompt ChatGPT with the region BEG END.
It then displays the results in a separate buffer `chatgpt-api-buffer'."
  (interactive "r")
  (chatgpt-api-prompt (buffer-substring BEG END)
                  ;; Show the results if not already being viewed
                  (chatgpt-api-show-results-buffer-if-active)))

;;;###autoload
(defun chatgpt-api-prompt-region-and-replace (BEG END)
...
(defun chatgpt-api--append-to-prompt (prompt comment-str)
...
(defun chatgpt-api--extract-text-from-query (query-result)
  ...
  (condition-case err
      (thread-last query-result
                   ...
                   string-trim)
    (error
     (signal 'chatgpt-api-parsing-error err))))

(defun chatgpt-api--parse-response (status callback)
  ...
  (goto-char 0)
  (re-search-forward "^$")
  (funcall callback (chatgpt-api--extract-text-from-query (json-read))))

(defun chatgpt-api--query-open-api (prompt callback)
	 ...
	 (url-request-extra-headers `(("Content-Type" . "application/json")
         ...
         (url-request-data (json-encode
			    `(("model" . "text-davinci-003")
			      ("prompt" . ,prompt)
			      ("max_tokens" . ,chatgpt-api-max-tokens)
			      ("temperature" . 0)))))
...
you have the correctly set the OPENAI_API_KEY variable"
    ...
    (url-retrieve
     ...
     'chatgpt-api--parse-response
     (list callback))))

(provide 'chatgpt-api)
;;; chatgpt-api.el ends here
